...
import { uploadProfilePhoto, signOutUser } from "@goshats/firebase";
import {
  updateRider,
  deleteRider,
} from "@goshats/firebase/src/firestore/riders";
...
export default function PersonalInfoScreen() {
  ...
  const [form, setForm] = useState({
    surname: riderProfile?.surname ?? "",
    otherName: riderProfile?.otherName ?? "",
    email: riderProfile?.email ?? "",
    phone: riderProfile?.phone ?? "",
  });
  const [errors, setErrors] = useState<Record<string, string>>({});

  const fullName = `${form.otherName} ${form.surname}`.trim() || "Rider Name";

  const handlePickPhoto = async () => {
    const perm = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!perm.granted)
      return Alert.alert("Permission denied", "Allow photo access in Settings.");
    ...
    try {
      ...
      await updateRider(user.uid, { profilePhotoUrl: downloadUrl });
      if (riderProfile) {
        setRiderProfile({ ...riderProfile, profilePhotoUrl: downloadUrl });
      }
    } catch {
